...
void setup() {
  ...
  //Serve webroot
  server.serveStatic("/", SD, "/ESP32EJS/webroot/").setDefaultFile("index.html");

  // Serve ROM list as JSON at /romlist
  server.on("/romlist", HTTP_GET, [](AsyncWebServerRequest *request) {
    File root = SD.open("/ESP32EJS/webroot/roms");
    String list = "[";
    bool firstROM = true;
  ...
  // Start server and show info on the screen
  server.begin();
